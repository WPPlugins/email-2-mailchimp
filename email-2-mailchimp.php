<?php
/*
Plugin Name: eMail 2 MailChimp
Plugin URI: http://wordpress.org/plugins/email-2-mailchimp/
Description: Плагин подписывает email комментатора в выбранный лист в сервисе MailChimp.
Author: Арсен Ибрагимов
Author URI: http://ibragimov.me/
Version: 1.0.9
*/

/*  Copyright 2013 Arsen Ibragimov (email: barsnur@gmail.com)
...
if ( ! class_exists( 'MCAPI' ) )
	//require_once( 'http://ibragimov.me/MCAPI.class.php' );
...
if ( ! class_exists( 'TGM_MailChimp_Comment_Optin' ) ) {
	...
	class TGM_MailChimp_Comment_Optin {
		...
		public function handle_api_request() {
			...
			if ( isset( $_POST[sanitize_key( 'tgm_mc_action' )] ) && 'get_mc_api_key' == $_POST[sanitize_key( 'tgm_mc_action' )] ) {
				...
				if ( ! empty( $api->errorCode ) ) {
					...
					//$message = sprintf( __( 'Ошибка! MailChimp не принял введённый API ключ. MailChimp gave this response: <p><em>%s</em></p> Please try entering your API key again.', 'tgm-mc-optin' ), $api->errorMessage );
					$message = sprintf( __( 'Ошибка! MailChimp не принял введённый API ключ. Пожалуйста попробуйте ввести API ключ ещё раз.', 'tgm-mc-optin' ), $api->errorMessage );
		...
		public function admin_menu() {
			...
			add_options_page( __( '"eMail 2 MailChimp": Настройки', 'tgm-mc-optin' ), __( 'eMail 2 MailChimp', 'tgm-mc-optin' ), 'manage_options', 'tgm-mailchimp-comment-settings', array( &$this, 'settings_page' ) );
		...
		public function settings_page() {
			...
			<div class="onars-mc-settings wrap">
				...
				<div style="text-align:center;margin-top:20px;">
					Плагин "eMail 2 MailChimp" версия 1.0.9<br />
					2013 &copy; Разработал <a href="http://ibragimov.me" target="_blank">Арсен Ибрагимов</a><br />
					<img src="<?=plugin_dir_url( __FILE__ )?>lib/images/MailChimp.png" style="margin-top:10px;" />
				</div>
			</div>
			<?php
		...
		public function get_api_creds() {
			...
			?>
			<div class="onars-mc-header">
				<div class="content">
					<p style="font-weight:bold"><?php _e( 'Для работы с плагином необходимо ввести MailChimp API ключ.', 'tgm-mc-optin' ); ?></p>
					<p><a href="http://admin.mailchimp.com/account/api-key-popup" class="button-secondary get-auth" target="_tab"><?php _e( 'Нажмите здесь чтобы получить API Key', 'tgm-mc-optin' ); ?></a>
					</p>
					<p></p>
				</div>
			</div>
			<div class="onars-mc-table">
				<div class="content">
					<table class="form-table">
						<tbody>
							<tr valign="middle">
								<th scope="row" style="text-align:right">
									<label><?php echo esc_attr__( 'Введите MailChimp API ключ:', 'tgm-mc-optin' ); ?></label>
								</th>
								<td>
									<input id="onars-mc-apikey" type="text" name="tgm_mc_get_api_key" size="50" value="" />
								</td>
							</tr>
						</tbody>
					</table>
					...
					<?php 
						submit_button(
							__( 'Авторизоваться в MailChimp с этим ключом API', 'tgm-mc-optin' ), // Button text
		...
		public function get_form_options() {
			...
			<div class="onars-mc-header">
				<div class="content">
					<?php printf( __( '<p>%1$s <strong>%2$s</strong> <a href="%3$s" class="onars-mc-logout button-secondary">Выйти</a></p>', 'tgm-mc-optin' ), 'Вы вошли как: ', $tgm_mc_options['username'], add_query_arg( array( 'page' => 'tgm-mailchimp-comment-settings', 'tgm_mc_action' => 'logout', 'tgm_mc_logout_nonce' => wp_create_nonce( 'tgm_mc_logout' ) ), admin_url( 'options-general.php' ) ) ); ?>
				</div>
			</div>
			<div class="onars-mc-table">
				<div class="content">
					<table class="form-table">
						<tbody>
							<tr valign="middle">
								<th scope="row" style="text-align:right">
									<label for="<?php echo $this->option; ?>[current_list_name]">В какой лист подписывать:</label>
								</th>
								<td>
									<?php
										$values = array();
										...
										echo '<select id="onars-mc-lists" name="' . $this->option . '[current_list_name]">';
											echo '<option value=",,,">Выберите лист</option>';
							...
							<tr valign="middle" style="display:none">
								...
								<td>
									<input id="onars-mc-show" type="checkbox" name="<?php echo $this->option; ?>[show]" value="<?php echo $tgm_mc_options['show']; ?>" <?php checked( $tgm_mc_options['show'], 0 ); ?> />
								</td>
							</tr>
							<tr valign="middle" style="display:none">
								<th scope="row">
								...
								<td>
									<input id="onars-mc-check-text" type="text" name="<?php echo $this->option; ?>[check_text]" size="55" value="<?php echo $tgm_mc_options['check_text']; ?>" />
								</td>
							</tr>
							<tr valign="middle" style="display:none">
								<th scope="row">
								...
								<td>
									<input id="onars-mc-subscribed-text" type="text" name="<?php echo $this->option; ?>[subscribed_text]" size="55" value="<?php echo $tgm_mc_options['subscribed_text']; ?>" />
								</td>
							</tr>
							<tr valign="middle" style="display:none">
								<th scope="row">
								...
								<td>
									<input id="onars-mc-pending-text" type="text" name="<?php echo $this->option; ?>[pending_text]" size="55" value="<?php echo $tgm_mc_options['pending_text']; ?>" />
								</td>
							</tr>
							<tr valign="middle" style="display:none">
								<th scope="row">
								...
								<td>
									<input id="onars-mc-admin-text" type="text" name="<?php echo $this->option; ?>[admin_text]" size="55" value="<?php echo $tgm_mc_options['admin_text']; ?>" />
								</td>
							</tr>
							<tr valign="middle" style="display:none">
								<th scope="row">
								...
								<td>
									<input id="onars-mc-clear" type="checkbox" name="<?php echo $this->option; ?>[clear]" value="<?php echo $tgm_mc_options['clear']; ?>" <?php checked( $tgm_mc_options['clear'], 1 ); ?> />
							...
							<tr valign="middle" style="display:none">
								...
								<td>
									<input id="onars-mc-check" type="checkbox" name="<?php echo $this->option; ?>[check]" value="<?php echo $tgm_mc_options['check']; ?>" <?php checked( $tgm_mc_options['check'], 1 ); ?> />
					...
					<?php 
						submit_button(
							__( 'Сохранить изменения', 'tgm-mc-optin' ), // Button text
							'secondary', // Button class
		...
		public function comment_form() {
			...
			/*
			if ( 'admin' == $status ) {
				echo '<p class="onars-mc-subscribe" ' . $clear . '>' . $tgm_mc_options['admin_text'] . '</p>';
			}
			elseif ( 'subscribed' == $status ) {
				echo '<p class="onars-mc-subscribe" ' . $clear . '>' . $tgm_mc_options['subscribed_text'] . '</p>';
			}
			elseif ( 'pending' == $status ) {
				echo '<p class="onars-mc-subscribe" ' . $clear . '>' . $tgm_mc_options['pending_text'] . '</p>';
			}
			else {
				echo '<p class="onars-mc-subscribe" ' . $clear . '>';
					echo '<input type="checkbox" name="tgm_mc_get_subscribed" id="onars-mc-get-subscribed" value="subscribe" style="width: auto;" ' . $checked . ' />';
		...
		public function populate_list() {
			...
			$api->listSubscribe($tgm_mc_options['apikey'], $tgm_mc_options['current_list_id'], $email, $name, plugins_url());
